...
exports.verifyOtpHandler = async (req, res) => {
    ...
    try {
        console.log("Request received - Email:", email, "OTP:", otp); // Debug log
        ...
        if (user.otp === otp) {
            ...
            user.token = token;
            await user.save();
            console.log("OTP verification successful, token:", token); // Debug log

            return res.status(200).json({ message: "OTP verified successfully", token });
        } else {
            console.log("Invalid OTP provided:", otp, "Expected OTP:", user.otp); // Debug log
            return res.status(400).json({ message: "Invalid OTP" });
        }
    } catch (err) {
